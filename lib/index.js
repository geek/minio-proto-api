...
async function register (server, options) {
  ...
  await initDatabase(options.db, DbSchema.toString(), options.accounts);
...
async function initDatabase (dbOptions, schema, accounts) {
...
async function createConnection (options, parentBarrier) {
  const barrier = parentBarrier || new Barrier();
  ...
  connection.connect((err) => {
    ...
    setTimeout(() => {
      console.log('retrying connection to mysql');
