...
module.exports = {
  Query: {
    bridge: async (root, { bridgeId }, request) => { // eslint-disable-line require-await
      const sql = 'SELECT bridgeId, container1Id, container2Id, accountId, username, namespace, name, directoryMap FROM bridges WHERE bridgeId = ? AND accountId = ?';
    ...
    listBridgesByAccount: async (root, options, request) => { // eslint-disable-line require-await
      const sql = 'SELECT bridgeId, container1Id, container2Id, accountId, username, namespace, name, directoryMap FROM bridges WHERE accountId = ?';
      const accountId = Common.getAccountId(request);
      const pool = request.server.app.mysql;
      const barrier = new Barrier();
  ...
  Mutation: {
    createBridge: async (root, bridge, request) => {
      const sql = 'INSERT INTO bridges (bridgeId, accountId, username, namespace, name, sshKey, accessKey, secretKey, directoryMap) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)';
      const pool = request.server.app.mysql;
      const barrier = new Barrier();
      const bridgeId = Uuid.v4();
      const accountId = Common.getAccountId(request);

      const args = [bridgeId, accountId, bridge.username, bridge.namespace, bridge.name,
