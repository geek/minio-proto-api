...
const Resolvers = {
  ...
  Mutation: {
    createAccount: async (root, { id, isAdmin }, request) => {
      ...
      const pool = request.server.app.mysql;
      const results = await Common.doQuery(sql, [id, isAdmin ? 1 : 0], pool);
    ...
    deleteAccount: async (root, { id }, request) => {
      ...
      const getSql = 'CALL get_account(?)';
      const pool = request.server.app.mysql;
      const [getResults] = await Common.doQuery(getSql, [id], pool);

      if (getResults.length === 0) {
        return Boom.notFound(`Account with ID: ${id} not found`);
      }

      const deleteSql = 'CALL delete_account(?)';
      const results = await Common.doQuery(deleteSql, [id], pool);

      // Sanity check. This should always be one.
      if (results.affectedRows !== 1) {
        throw Boom.internal(`Delete affected ${results.affectedRows} accounts`);
      }

      return createAccountFromDb(getResults[0]);
    },
